...
def generate_detailed_ai_report(patient_id: str, disease_name: str, deviations: list[str]) -> str:
    ...
    """
    if not GEMINI_MODEL:
        raise RuntimeError("Gemini AI explanation is unavailable because GEMINI_API_KEY is not configured.")
...
"""
    try:
        ...
        raise RuntimeError(f"Gemini AI explanation failed: {str(e)}") from e

    raise RuntimeError("Gemini AI explanation returned an empty response.")
...
def generate_health_report(
    ...
    for sym, detail in sorted(z_scores.items(), key=lambda x: abs(x[1].get("z_score", 0)), reverse=True)[:4]:
        ...
        if abs(z) >= 1.0:
            label = sym.replace("_", " ").title()
            suffix = "σ desde tu línea base" if is_es else "σ from baseline"
            top_deviations.append(f"{label}: {z:+.1f}{suffix}")
    ...
    if forecast:
        if forecast[-1] > forecast[0]:
            forecast_trend = "ligera tendencia al alza del riesgo en los próximos 3 días" if is_es else "slight upward risk trend over the next 3 days"
        elif forecast[-1] < forecast[0]:
            forecast_trend = "tendencia de riesgo en mejoría en los próximos 3 días" if is_es else "improving risk trend over the next 3 days"
        else:
            forecast_trend = "tendencia de riesgo estable en los próximos 3 días" if is_es else "stable risk trend over the next 3 days"
    ...
    closing_note = (
        "Nota: Este resumen es generado por un asistente de IA para ayudarte a entender tus datos. No reemplaza el consejo médico de tu equipo de atención."
        if is_es else
        "Note: This summary is generated by an AI assistant to help you understand your data. It does not replace medical advice from your care team."
    )
...
5. The report language MUST be {"Spanish" if is_es else "English"}.
6. MUST end with exactly: "{closing_note}"

Write in a warm, supportive tone. Avoid jargon. Keep each paragraph concise."""
        try:
            response = GEMINI_MODEL.generate_content(prompt)
            if response and response.text:
                candidate_summary = response.text.strip()
                # Reject incomplete or cut-off Gemini output and fall back to the deterministic summary.
                if candidate_summary.endswith(closing_note):
    ...
    if not ai_summary:
        risk_messages = {
            "LOW": "Tus señales de síntomas se mantienen en gran medida dentro de tu rango habitual. Continúa con el monitoreo regular y tus rutinas de autocuidado." if is_es else "Your symptom signals are largely within your personal baseline range. Keep up with regular monitoring and self-care routines.",
            "MODERATE": "Algunas de tus señales de síntomas han subido de forma moderada por encima de tu línea base personal. Vale la pena comentarlo en tu próxima cita." if is_es else "Some of your symptom signals have risen moderately above your personal baseline. This is worth discussing at your next appointment.",
            "HIGH": "Varias señales de síntomas están elevadas por encima de tu línea base personal. Recomendamos contactar pronto a tu equipo de atención para revisar tus datos recientes." if is_es else "Several symptom signals are elevated above your personal baseline. We recommend contacting your care team soon to review your recent data.",
            "CRITICAL": "Tus señales de síntomas muestran una elevación importante. Comunícate con tu equipo de atención lo antes posible; no es una alerta de emergencia, pero sí una recomendación para revisión." if is_es else "Your symptom signals show significant elevation. Please contact your care team promptly — this is not an emergency alert, but a recommendation to seek review.",
            "INSUFFICIENT_DATA": "Aún no hay suficientes datos registrados para un análisis completo de señales. Intenta registrar tus síntomas a diario para obtener una imagen más precisa." if is_es else "You don't have enough logged data yet for a full signal analysis. Try to log symptoms daily for a more accurate picture.",
        }
        dev_text = (
            (f"Tus principales cambios de señal: {', '.join(top_deviations[:2])}. " if is_es else f"Your top signal changes: {', '.join(top_deviations[:2])}. ") if top_deviations
            else ("Todas las señales se ven cerca de tu línea base personal." if is_es else "All signals appear near your personal baseline.")
        )
        dominant_domain = ["mobility", "sleep", "cognitive"][([mobility_score, sleep_score, cognitive_score].index(max(mobility_score, sleep_score, cognitive_score)))] if fis_composite > 0 else ""
        dominant_domain_label = {
            "mobility": "movilidad" if is_es else "mobility",
            "sleep": "sueño" if is_es else "sleep",
            "cognitive": "cognición" if is_es else "cognitive",
        }.get(dominant_domain, dominant_domain)
        fi_text = (
            (
                f"Tu puntaje de impacto funcional es {round(fis_composite * 100)}%, "
                f"y {dominant_domain_label} es el dominio más afectado."
            ) if is_es else (
                f"Your functional impact score is {round(fis_composite * 100)}%, "
                f"with {dominant_domain_label} as the most affected domain."
            )
        ...
        ai_summary = (
            ...
            f"{('⚠ Señales de alerta detectadas: ' + ', '.join(red_flags) + '. Compártelo con tu médico.' if is_es else '⚠ Red flags detected: ' + ', '.join(red_flags) + '. Please share this with your doctor.') if red_flags else ('No se detectaron señales de alerta en este período.' if is_es else 'No red flags detected in this period.')}\n\n"
            f"{closing_note}"
        )
    ...
    return {
        ...
        "disclaimer": (
            "Este informe es una ayuda estructurada de monitoreo generada por RareSignal IA. NO constituye consejo médico, diagnóstico ni pronóstico. Todas las decisiones clínicas siguen estando a cargo de profesionales de salud calificados."
            if is_es else
            "This report is a structured monitoring aid generated by RareSignal AI. It does NOT constitute medical advice, diagnosis, or prognosis. All clinical decisions remain with qualified healthcare providers."
        ),
    }
